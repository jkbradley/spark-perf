...
def run_tests():
    ...
    #    'python file that ends in .py', default="%s/config.py" % PROJ_DIR)
    ...
    #args_config_file = str("%s/config.py" % PROJ_DIR)
    #args_additional_make_distribution_args = str("")

    #assert args_config_file.endswith(".py"), "config filename must end with .py"

    ## Check if the config file exists.
    #assert os.path.isfile(args_config_file), ("Please create a config file called %s (you probably "
    #    "just want to copy and then modify %s/config/config.py.template)" %
    #    (args_config_file, PROJ_DIR))
    ...
    if should_prep_spark:
        assert config.SPARK_COMMIT_ID is not "", \
            ("Please specify SPARK_COMMIT_ID in %s" % args_config_file)
    ...
    if config.IS_MESOS_MODE:
        ...
        cluster = spark_build_manager.get_cluster(
            ...
            additional_make_distribution_args=args_additional_make_distribution_args)
